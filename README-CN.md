...
API 端点表单提供「值无引号」勾选框（默认关闭，与报表 JSON 导出的同名选项共用同一实现）。开启后 JSON 输出中**所有值均不加引号**（含字符串，如 `张三`、数字字符串 `007`），仅键保留引号——不再区分数字与字符串；输出不再保证是严格合法 JSON，面向宽松下游解析器。对默认输出结构与自定义 JSON 模板同时生效；CSV 输出不受影响；静态缓存 `.json` 变体随该开关自动失效重建（配置变更即失效，下次请求重建）。
