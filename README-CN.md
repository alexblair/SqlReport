...
命中勾选形态的值在裸输出前执行合法化转换（去逗号 → 去前导 `+` → 去前导零，文本级操作、绝不过 float/int、防精度丢失），转换结果经 RFC 8259 number 语法兜底校验，不合法则回退带引号——**输出永远合法 JSON（RFC 8259）**。原生数字（int/float/Decimal）恒按数字输出，不因面板勾选变化；含非数字内容的值（日期、空串、`true`/`false`/`null` 等）永远带引号。对默认输出结构与自定义 JSON 模板同时生效（面板开启时模板仍保留渲染后合法性校验——与旧全裸模式跳过校验相反）；CSV 输出不受影响；静态缓存 `.json` 变体随勾选变化自动失效重建（已纳入 config_version 计算）。旧 `json_no_quotes=1`（数据库列与 URL 参数）仍可用，运行时等价映射为面板全开；面板 3 项勾选（十进制/科学/千分位）以 `smart_quote_flags` 位图落库。
