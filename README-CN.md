...
| **悬浮表头** | 报表结果超长滚动时表头吸附在表格容器顶部，向下滚动仍可见列名 |
| **多字段排序** | 点击列头排序，支持多列组合排序，带排序管理面板（添加/删除/调序） |
| **多字段筛选** | 10 种操作符（包含/不包含/等于/不等于/大于/小于/≥/≤/为空/非空）；筛选值支持 `*` 通配、英文逗号多值（或）、`\` 转义，报表页/导出/API/审计页共用同一语法 |
| **字段设置** | 拖拽排序、显示/隐藏列，自由控制表格展示字段 |
| **CSV 导出** | 一键导出完整查询结果，UTF-8 BOM 确保 Excel 正确识别中文 |
| **JSON 导出** | 支持 JSON 格式导出，可选智能去引号面板（十进制数字/科学计数法/千分位数字），输出永远合法 JSON |
...
| **站点标识** | favicon 可配置（内置 / 纯色生成 / 自定义上传 PNG·ICO ≤256KB）+ 可选标签页环境前缀（如 `[DEV] `），一眼区分多套部署；存储在实例本地 SQLite 文件，多个部署共用一个配置库时互不串扰；保存后刷新页面立即生效 |
| **三层查询缓存** | L1 进程内存（300s TTL）→ L2 Redis 快照（版本化键 + 分布式锁）→ L3 数据库直连（Redis 兜底） |
| **报表定时执行** | 独立定时任务，可绑定单张或多张报表（按序执行），支持排除规则（静默窗口：`dow`/`tod`/`date`/`date_range` 叶子、AND/OR 嵌套、多规则并行），错过执行补偿策略（跳过 / 当天补跑一次），任务级审计开关（默认关），连续失败 5 次熔断，管理页 `/config/scheduler` 支持手动触发，全程审计留痕 |
| **缓存保活** | 对开启缓存的报表，快照剩余有效期低于提前量秒数时后台主动重建 Redis 快照，首个请求不再承担重建耗时（需报表开启缓存且 Redis 可用） |
...
- `scheduler.enable` — `true` 时随 Web 服务启动后台报表调度器；`false`（默认）不启动。全局停用时已配置的定时任务保留但不会自动执行，`/config/scheduler` 页面顶部显示横幅提示
- 任务在 `/config/scheduler` 统一管理：每个任务有名称，可绑定一张或多张报表（`schedule_reports` 关联表，按 `order_index` 顺序执行；单绑定可独立停用），一张报表也可出现在多个任务中
- 排除规则（静默窗口）定义任务「何时不执行」：规则以 JSON 树存储（`exclusions` 列），支持 `dow` / `tod`（跨午夜感知）/ `date` / `date_range` 四种叶子、AND/OR 嵌套组合、多规则并行（任一命中即静默）。命中时不执行、标记 `skipped` 并推进 `next_run_at`；非法规则一律不静默（fail-open）。手动触发不受排除影响
- 任务级审计开关（`audit_enabled`，默认关）：关闭时不写任何 scheduler 类型审计；开启时执行 / 跳过 / misfire 分别记 `scheduled_run` / `scheduled_skip` / `scheduled_misfire`
- 调度行为在报表编辑表单的「⏰ 定时执行」折叠区按报表配置：间隔分钟数或每天固定时刻、错过执行补偿策略、启用开关
- 调度器启动时对过期任务做一次扫描：错过时刻命中排除窗口视为正确跳过（仅推进，不补跑）；否则 `interval` 任务合并为一次补跑，`daily` 任务按补偿策略处理（`skip` 推进到明天并写入 `scheduled_misfire` 审计记录，`run_once` 当天最多补跑一次）
- 连续失败 5 次后任务自动熔断停发；在 `/config/scheduler` 手动触发可绕过熔断，手动执行成功后自动恢复派发
- 缓存保活（「♻ 缓存保活」折叠区）要求报表开启 Redis 缓存；快照剩余有效期低于提前量秒数时由调度器后台重建
...
API 端点表单提供「智能去引号」复选面板（默认全部不勾选，与报表 JSON 导出的同名选项共用同一实现）。它取代了旧「值无引号」开关（已删除）：不再对**所有**值一律去掉引号，只对命中勾选形态的字符串值去引号：

| 形态 | 示例 |
|---|---|
| 十进制数字（含正负号） | `9.999`、`-1.5`、`007`→`7`、`+5`→`5` |
| 科学计数法 | `1e5`、`1E+5`、`1e-3`、`007e5`→`7e5` |
| 千分位数字 | `1,000`→`1000`、`-1,234.50`→`-1234.50` |

命中勾选形态的值在裸输出前执行合法化转换（去逗号 → 去前导 `+` → 去前导零，文本级操作、绝不过 float/int、防精度丢失），转换结果经 RFC 8259 number 语法兜底校验，不合法则回退带引号——**输出永远合法 JSON（RFC 8259）**。原生 int/float 恒按数字输出，不因面板勾选变化；Decimal 列（MySQL DECIMAL）在面板任一数字形态勾选（flags>0，含仅千分位）时数值化裸输出，未勾选时保持带引号；含非数字内容的值（日期、空串、`true`/`false`/`null` 等）永远带引号。对默认输出结构与自定义 JSON 模板同时生效（面板开启时模板仍保留渲染后合法性校验——与旧全裸模式跳过校验相反）；CSV 输出不受影响；静态缓存 `.json` 变体随勾选变化自动失效重建（已纳入 config_version 计算）。旧 `json_no_quotes=1` URL 参数仍兼容等价面板全开；数据库旧列由迁移 15 一次性转换（=1 → 面板全开 0b111）并重置为 0，不再运行期驱动行为（面板是引号的唯一控制）；面板 3 项勾选（十进制/科学/千分位）以 `smart_quote_flags` 位图落库。

API 端点表单还提供**接口说明预览**：「接口说明」输入框支持 Markdown（标题/列表/表格/代码块/```mermaid 流程图），带「预览接口说明」按钮实时渲染（`POST /config/api-endpoints/description-preview`，镜像报表备注预览，纯渲染无落库、无数据库依赖）；编辑预览与报表页展示共用同一 `render_markdown` 单一来源。
...
- 「⏰ 定时执行」折叠区：启用自动执行、间隔分钟数或每天 HH:MM、错过补偿策略（跳过 / 补跑一次）；保存时同步任务行（参数未变则保持既有节拍）。多报表绑定、排除规则编辑器、审计开关等完整任务管理在 `/config/scheduler` 进行
- 「♻ 缓存保活」折叠区：到期前主动重建 Redis 快照（需开启 Redis 缓存）；提前量秒数可配，0 = 不保活
- 报表列表徽标：⏰ = 已配置且启用定时任务，♻ = 已开启缓存保活
...
- 应用全量输出护栏：关闭全量输出且结果超过 max_rows 时导出被截断，带响应头 `X-Export-Truncated: true`，并在文件内留标记（CSV 末尾追加 `# ...` 说明行；JSON 顶层追加 `_meta: {"truncated": true, "max_rows": N}` 对象）
