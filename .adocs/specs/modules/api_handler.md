...
contract_id: MOD-API_HANDLER
version: 1.0
depends_on: [app_config, auth, db, export, json_template, query_executor, redis_cache, report, result_transform, static_cache]
...
- `_resolve_params(endpoint, method, body, query_params, headers=None)`：预设规则 + POST/GET 覆盖合并。
...
server._handle_api → api_handler.handle_api_request(conn, path, method, headers, body, query_params, client_ip)
  ...
  └─ 普通链路 → _run_normal_api_request
       ├─ _execute_api_query: 加载报表/连接池 + _resolve_params + execute_report
